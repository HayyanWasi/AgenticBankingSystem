...
export default function KycPage() {
  ...
  // The state memory tray inside the component where it belongs
  ...
  const handleKycSubmission = async () => {
    ...
    try {
      ...
      if (res.status === "success" || res.success === true) {
        setStep(3); // Advance to success display screen

        // Hold for 3 seconds so user can read screen, then redirect home
        setTimeout(() => {
          navigate("/dashboard");
        }, 3000);
      } else {
        setError(res.message || "KYC submission failed.");
      }
    } catch (err: any) {
      setError(err.message || "Network error. Server might be offline.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-[20px] md:p-[40px] max-w-[900px] mx-auto">
        ...
        <div className="flex items-center justify-between mb-[4px]">
          {verificationSteps.map((s, i) => (
            <div key={i} className="flex flex-col items-center flex-1 relative">
              ...
              <div className={`w-[36px] h-[36px] rounded-full flex items-center justify-center relative z-10 transition-colors ${i <= step ? 'bg-secondary text-on-secondary' : 'bg-surface-container-high text-on-surface-variant'
                }`}>
      ...
      {/* Step 0: Personal Info Data Fields Collection */}
      {step === 0 && (
          ...
          <form onSubmit={(e) => { e.preventDefault(); setStep(1); }} className="flex flex-col gap-[16px]">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-[16px]">
              <div className="flex flex-col gap-[4px] md:col-span-2">
                <label className="font-[var(--font-mono)] text-[12px] text-on-surface-variant tracking-[0.05em]">Full Name</label>
                <input
                  className="w-full bg-surface-bright border border-outline-variant rounded-[8px] py-[12px] px-[16px] font-[var(--font-body)] text-[16px] text-on-surface focus:outline-none focus:border-secondary"
                  value={fullName}
              ...
              <div className="flex flex-col gap-[4px]">
                <label className="font-[var(--font-mono)] text-[12px] text-on-surface-variant tracking-[0.05em]">Phone Number</label>
                <input
                  type="tel"
            ...
            <div className="flex flex-col gap-[4px]">
              <label className="font-[var(--font-mono)] text-[12px] text-on-surface-variant tracking-[0.05em]">ID Card Number</label>
              <input
                className="w-full bg-surface-bright border border-outline-variant rounded-[8px] py-[12px] px-[16px] font-[var(--font-body)] text-[16px] text-on-surface focus:outline-none focus:border-secondary"
                placeholder="e.g. ID-445511"
                value={idCardNum}
      ...
      {/* Step 1: Document Upload Screen Component */}
      {step === 1 && (
        <div className="bg-surface-container-lowest rounded-[16px] border border-outline-variant/10 p-[24px] animate-fade-in-up">
      ...
      {/* Step 2: Form Submission Interceptor Node */}
      {step === 2 && (
        <div className="bg-surface-container-lowest rounded-[16px] border border-outline-variant/10 p-[24px] animate-fade-in-up">
          ...
          <div className="flex flex-col items-center py-[32px]">
            ...
            <button
              ...
              disabled={loading}
              className="bg-secondary text-on-secondary font-[var(--font-mono)] text-[12px] font-medium tracking-[0.05em] px-[32px] py-[14px] rounded-[8px] cursor-pointer border-none flex items-center gap-[8px] disabled:bg-outline-variant"
            >
              <span className="material-symbols-outlined" style={{ fontSize: '18px' }}>
                {loading ? 'sync' : 'fingerprint'}
              </span>
              {loading ? "Submitting Core Payload..." : "Start Scan & Submit"}
            </button>
          </div>
        </div>
      )}

      {/* Step 3: Server Validated Visual State Display */}
      {step === 3 && (
